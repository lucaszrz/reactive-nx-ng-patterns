 ...
 * Usage: <ng-template appTableColumn="columnName" let-value let-row="row">...</ng-template>
 */
@Directive({
...
export class TableColumnDirective {
  @Input('appTableColumn') columnName!: string;
...
@Component({
  ...
  imports: [
    ...
    MatColumnDef,
    MatHeaderCell,
    MatHeaderCellDef, MatCell, MatCellDef, TitleCasePipe, MatIconButton, MatIcon, MatHeaderRow, MatHeaderRowDef, MatRow, MatRowDef, MatPaginator, AsyncPipe
  ],
  template: `
    <div class="table-container">
        ...
        <header class="toolbar">
          ...
          <div class="button-group">
            <button mat-flat-button
                    color="primary"
                    (click)="triggerManualReload()">Search
            </button>
        ...
        <table mat-table
          ...
          @for (col of displayedColumns; track col) {
            <ng-container [matColumnDef]="col">
              <th mat-header-cell
                  *matHeaderCellDef
                  mat-sort-header> {{ col | titlecase }}
              </th>
              <td mat-cell
                  *matCellDef="let row">
                @let customTemp = getCustomTemplate(col);
                @if (customTemp) {
                  <ng-container [ngTemplateOutlet]="customTemp"
                                [ngTemplateOutletContext]="{ $implicit: row[col], row: row }">
                  </ng-container>
                } @else {
                  {{ row[col] }}
                }
          ...
          <tr mat-header-row
              *matHeaderRowDef="allDisplayedColumns()"></tr>
          <tr mat-row
              *matRowDef="let row; columns: allDisplayedColumns();"></tr>
        </table>
      ...
      <footer class="table-footer">
        <span class="total-count">Total: {{ totalElements() }} {{ elementName }}</span>
        <mat-paginator
...
export class GenericTable implements OnInit {
  ...
  @ContentChild('customSearchTemplate') customSearchTemplate?: TemplateRef<any>;
  @ContentChildren(TableColumnDirective) customColumnTemplates!: QueryList<TableColumnDirective>;
  ...
  showInitialMessage = computed(() => this.isInitialState() && this.dataSource().length === 0);
  showNoDataMessage = computed(() => !this.isInitialState() && this.dataSource().length === 0 && !this.isLoading());
  ...
  getCustomTemplate(colName: string): TemplateRef<any> | null {
    return this.customColumnTemplates?.find(t => t.columnName === colName)?.template || null;
  }
